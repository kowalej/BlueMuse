    ...
    public class MuseAccelerometerSamples
    {
        public static double[,] DecodeAccelerometerSamples(string bits)
        {
            ...
            for (int i = 0; i < Constants.MUSE_ACCELEROMETER_SAMPLE_COUNT; i++)
            {
                ...
                {
                    samples[i, j] = PacketConversion.ToInt16(bits, 16 + (((i * Constants.MUSE_ACCELEROMETER_CHANNEL_COUNT) + j) * 16)); // Initial offset by 16 bits for the timestamp.
                    samples[i, j] *= Constants.MUSE_ACCELEROMETER_SCALE_FACTOR;
                }
            }
            return samples;
        }
